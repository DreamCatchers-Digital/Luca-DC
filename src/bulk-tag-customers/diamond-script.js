...
async function downloadAndProcessResults(url) {
  const outputPath = path.join(__dirname, 'bulk_results_150k.jsonl');
  ...
  for await (const line of rl) {
    ...
    if (!node.id || !node.amountSpent) {
      console.warn('⚠️ Skipping row without ID or amountSpent:', line);
      continue;
    }

    const amountSpent = parseFloat(node.amountSpent.amount || '0');
    if (amountSpent < 150000) continue;
    ...
    await new Promise((r) => setTimeout(r, 300)); // Shopify rate limit safe
  }
